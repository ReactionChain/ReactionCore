...
const char * const BITCOIN_CONF_FILENAME = "reaction.conf";
const char * const BITCOIN_PID_FILENAME = "reactiond.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "reactiond -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Reaction
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Reaction
    // Mac: ~/Library/Application Support/Reaction
    // Unix: ~/.reaction
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Reaction";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Reaction";
#else
    // Unix
    return pathRet / ".reaction";
#endif
#endif
}
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty reaction.conf if it does not excist
...
std::string CopyrightHolders(const std::string& strPrefix, unsigned int nStartYear, unsigned int nEndYear)
{
    ...
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Reaction Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + strprintf(" %u-%u ", 2021, nEndYear) + "The Reaction Core developers";
    }
